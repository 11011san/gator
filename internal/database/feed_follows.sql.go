...
const getAllFeedWithUser = `-- name: GetAllFeedWithUser :many
...
type GetAllFeedWithUserRow struct {
	ID        uuid.UUID
	CreatedAt time.Time
	UpdatedAt time.Time
	FeedName  string
...
func (q *Queries) GetAllFeedWithUser(ctx context.Context) ([]GetAllFeedWithUserRow, error) {
	rows, err := q.db.QueryContext(ctx, getAllFeedWithUser)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetAllFeedWithUserRow
	for rows.Next() {
		var i GetAllFeedWithUserRow
		if err := rows.Scan(
			&i.ID,
			&i.CreatedAt,
			&i.UpdatedAt,
